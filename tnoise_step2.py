...
ChamberTemperatures = namedtuple(
    "ChamberTemperatures",
    [
...
def compute_T_load_attenuations(
    ...
    brightness_t_from_ambient = (
        t_ambient * (1 - wg_att) + wg_att * (t_ambient + t_avg_cross_guide) * 0.5
    ) * (1 - tot_att)
    brightness_t_from_cross_guide = t_avg_cross_guide * (1 - tot_att)
    brightness_t_port_a_total = (
        brightness_t_load_a + brightness_t_from_ambient + brightness_t_from_cross_guide
    )
    brightness_t_from_load_b = (
        t_b1 * (1 - wg_att) + wg_att * (t_b1 + t_avg_magic_t_polarimeter) * 0.5
    )
...
class LogLikelihood:
    ...
    def load_temp_at_detector(self, pwr_idx, unbalance=0.0):
        ...
        return (
            ...
            + cross_weight * (self.temperatures_a + self.temperatures_b) / 2 * unbalance
        )
...
def calc_wn_level(values):
    ...
    _, psd = welch(
        ...
        window="hann",
        nperseg=2.0 * SAMPLING_FREQUENCY_HZ,
        detrend="linear",
    )
...
def extract_average_values(
    power_data, dem_data, metadata, tnoise1_results, num, offsets=None
):
    ...
    if not regions:
        log.fatal(
            "unable to find {0} temperature steps in the data, only {1} were available".format(
                num, ", ".join([str(x) for x in region_lengths])
            )
        )
        sys.exit(1)
    ...
    if offsets is None:
        offsets = [
            metadata["detector_outputs"][-1]["{0}_adu".format(det)]
...
def y_factor_pairs(num_of_steps):
    ...
    return [x for x in product(range(num_of_steps), range(num_of_steps)) if x[0] < x[1]]
...
def y_factor_estimates(log_ln, unbalance):
    ...
    for idx1, idx2 in y_factor_pairs(len(log_ln.temperatures_a)):
        for pwr_idx in (0, 1, 2, 3):
            ...
            tnoise, gain = estimate_tnoise_and_gain(
                temp_a=temperatures[idx1],
    ...
    result.sort(key=lambda x: (x["detector_idx"], x["step_1_idx"], x["step_2_idx"]))
...
def assemble_results(polarimeter_name: str, log_ln: LogLikelihood):
    return {
        ...
        "title": (
            "Noise temperature analysis for polarimeter {0}".format(polarimeter_name)
        ),
...
def assemble_nonlinear_fit_results(
    polarimeter_name: str, log_ln: LogLikelihood, popt, pcov
):
...
def create_timestream_plot(log_ln, params, output_path):
    ...
    for pwr_idx in (0, 1, 2, 3):
        cur_pwr = np.array(
            [log_ln.voltages[pwr_idx][i] for i in range(len(log_ln.temperatures_a))]
        )
        (base_line,) = plt.plot(varying_t, cur_pwr, "-o", label=detector_name(pwr_idx))

        if not np.all(model_estimates[pwr_idx] == 0.0):
            plt.plot(
                varying_t,
                model_estimates[pwr_idx],
                alpha=0.5,
    ...
    plt.subplot(212)
    for pwr_idx, gain in enumerate(
        (best_fit.gain_q1, best_fit.gain_u1, best_fit.gain_u2, best_fit.gain_q2)
    ):
        plt.plot(
            varying_t,
            (pwr[pwr_idx] - model_estimates[pwr_idx]) / gain,
            "-o",
            linestyle="--",
        )
...
def create_model_match_plot(log_ln, params, output_path):
    ...
    for pwr_idx in (0, 1, 2, 3):
        ...
        plt.scatter(voltages, model_estimates[pwr_idx], label=detector_name(pwr_idx))
...
def create_tnoise_plot(log_ln, params, output_path):
    ...
    for y_estimate in params["y_factor_estimates"]:
        tnoise, gain, temp1, temp2, out1 = [
            y_estimate[x]
            for x in ("tnoise", "gain", "temperature_1", "temperature_2", "output_1")
        ]
...
def create_tnoise_matrix_plot(log_ln, params, output_path):
    ...
    for cur_idx, cur_det in enumerate(sorted(detectors)):
        ...
        cur_estimates = [x for x in y_factor_estimates if x["detector_name"] == cur_det]
        matr_size = (
            np.max(
                np.matrix(
                    [[x["step_1_idx"], x["step_2_idx"]] for x in cur_estimates]
                ).flatten()
            )
            + 1
        )
        ...
        for i in range(matr_size):
            ...
            bunch = [
                x["temperature_1"] for x in cur_estimates if x["step_1_idx"] == i
            ] + [x["temperature_2"] for x in cur_estimates if x["step_2_idx"] == i]
            if bunch:
        ...
        for (i, j), z in np.ndenumerate(tnoise_matr):
            if z > 0.0:
                ax.text(
                    ...
                    ha="center",
                    va="center",
                    path_effects=[PathEffects.withStroke(linewidth=1, foreground="b")],
                )
...
def nonlinear_fit_analysis(args, log_ln):
    try:
        popt, pcov = opt.curve_fit(
            log_ln,
            None,
            np.array(log_ln.voltages).flatten(),
    ...
    log.info(
        "results of the fit: %s",
        ", ".join(
            [
                "{0} = {1:.2f} ± {2:.2f}".format(n, popt[i], np.sqrt(pcov[i, i]))
                for i, n in enumerate(NONLINEAR_PARAM_NAMES)
            ]
        ),
    )

    return assemble_nonlinear_fit_results(args.polarimeter_name, log_ln, popt, pcov)
...
def only_q_analysis(args, log_ln):
    ...
    for pwr_idx in range(4):
        ...
        if len(temp) < 4:
            ...
            best_fit, pcov = np.polyfit(temp, log_ln.voltages[pwr_idx], 1, cov=True)
        ...
        result[tnoise_param_name] = {
            ...
            "std": np.sqrt(
                pcov[1, 1] / (best_fit[1] ** 2) + pcov[0, 0] / (best_fit[0] ** 2)
            ),
        }

        log.info(
            "best gain for PWR{0}: {1:.0f} pm {2:.0f} ADU/K, best Tnoise: {3:.1f} pm {4:.1f} K".format(
                pwr_idx,
...
def parse_arguments():
    ...
    parser.add_argument(
        "--phsw", type=str, default=None, help='State of the PHSW (e.g., "0101")'
    )
    parser.add_argument(
        "--offsets",
        type=str,
    ...
    parser.add_argument(
        "--analysis-method",
        type=str,
        default=list(ANALYSIS_METHODS.keys())[0],
        help="""Tell what kind of analysis should be done on the data.
                        Possible alternatives are: {0} (first one is the default).""".format(
            ", ".join(['"{0}"'.format(x) for x in ANALYSIS_METHODS.keys()])
        ),
    )
    parser.add_argument(
        "polarimeter_name",
...
def main():
    ...
    for idx, arr in enumerate(voltages):
        log.info(
            "voltages for PWR%d: %s", idx, ", ".join(["{0:.1f}".format(x) for x in arr])
        )
        log.info(
    ...
    save_parameters_to_json(
        ...
        output_file_name=os.path.join(args.output_path, "tnoise_step2_results.json"),
    )
