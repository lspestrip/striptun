...
def get_fft(sampling_frequency, data, n_chunks, detrend="linear", **kwargs):
    ...
    spectrogram = signal.spectrogram(
        cdata, sampling_frequency, window="hanning", axis=0, detrend=detrend, **kwargs
    )
...
def get_noise_characteristics(freq, fft, left_freq, right_freq, totalPWR=False):
    ...
    def get_parameters(freq, fft, left_freq, right_freq, totalPWR):
        ...
        ab, cov = np.polyfit(
            np.log(freq[:f_idx_left]), np.log(fft[:f_idx_left]), 1, cov=True
        )
        a, b = ab
        ...
        delta_c = np.sum(np.abs(np.log(fft[f_idx_right:]) - c), axis=0) / len(
            fft[f_idx_right:]
        )
        ...
        delta_fknee_ = (
            fknee_
            / np.abs(a)
            * np.sqrt(((c - b) / a) ** 2 * delta_a ** 2 + delta_b ** 2 + delta_c ** 2)
        )
        ...
        def get_right_number_of_decimals(x, delta_x):
            def get_new_x(x, new_num_dec):
                new_x = np.array(
                    [np.around(x[i], decimals=dec) for i, dec in enumerate(new_num_dec)]
                )
    ...
    if totalPWR == "stokes":
        ...
        (
            ...
            WN_levelQU,
            delta_WN_levelQU,
        ) = get_parameters(freq, fft[:, 1:], left_freq, right_freq, totalPWR=False)
        return (
            np.column_stack((fit_parI, fit_parQU)),
            np.append(fkneeI, fkneeQU),
            np.append(delta_fkneeI, delta_fkneeQU),
            np.append(alphaI, alphaQU),
            np.append(delta_alphaI, delta_alphaQU),
            np.append(WN_levelI, WN_levelQU),
...
def create_plots(
    ...
    **kwargs
    ...
    comulative_plots(
        title, freq, np.column_stack((fftDEM, fftPWR)), legend_labels, output_path, g
    )
    ...
    # Plot Q1, Q2 and Q
    title = (
        polarimeter_name + " PSD - " + " ".join(labelsDEM[0::3]) + " " + labelsSTOKES[1]
    )
    ...
    # Plot U1, U2 and U
    title = (
        polarimeter_name + " PSD - " + " ".join(labelsDEM[1:3]) + " " + labelsSTOKES[2]
    )
...
def main():
    ...
    # Calculate the PSD
    log.info(
        "Computing PSD with number-of-chunks={}, 1/f-upper-frequency={},".format(
            args.n_chunks, args.left_freq
        )
        + " WN-lower-frequency={}, detrend={}".format(args.right_freq, args.detrend)
    )

    freq, fftDEM, spectrogramDEM = get_fft(
        SAMPLING_FREQUENCY_HZ, dataDEM, args.n_chunks, detrend=args.detrend
    )
    (
        fit_parDEM,
        fkneeDEM,
        ...
        WN_levelDEM,
        delta_WN_levelDEM,
    ) = get_noise_characteristics(freq, fftDEM, args.left_freq, args.right_freq)
    [log.info("Computed fknee, alpha, WN_level for " + nam + " outputs") for nam in DEM]

    fftPWR, spectrogramPWR = get_fft(
        SAMPLING_FREQUENCY_HZ, dataPWR, args.n_chunks, detrend=args.detrend
    )[1:]
    (
        fit_parPWR,
        fkneePWR,
    ...
    save_parameters_to_json(
        ...
        output_file_name=os.path.join(
            args.output_path, "noise_characteristics_results.json"
        ),
    )
