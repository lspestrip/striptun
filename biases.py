...
def detector_type(metadata: Dict[str, Any]):
    ...
    if metadata["band"] != "W":
        raise ValueError(
            "Unknown band {0} for polarimeter STRIP{1:02d}".format(
                metadata["band"], metadata["polarimeter_number"]
            )
        )
...
def build_dict_from_results(pol_name: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        outputs = metadata["detector_outputs"]
        result["output"] = {
            "q1_adu": outputs[0]["q1_adu"],
            "q2_adu": outputs[0]["q2_adu"],
...
def main():
    ...
    params = build_dict_from_results(pol_name=args.polarimeter_name, metadata=metadata)
    params["data_url"] = args.input_url

    save_parameters_to_json(
        params=dict(params, **get_code_version_params()),
        output_file_name=os.path.join(args.output_path, "bias_configuration.json"),
    )
